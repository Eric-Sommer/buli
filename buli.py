...
export = df[['season',
             ...
             'end_rank',
             'diff16',
             'diff17']][(df.spieltag == spieltag) & (df.points_cum == scf_points)]
...
df = df.sort_values(by=['season', 'team', 'spieltag'])
df['5gamesnogoal'] = ((df['goals_for'] == 0) &
...
plotcases = df[['season',
                'team',
                'end_rank','diff16','diff17']][(df.spieltag == spieltag) &
...
df['goals_against_cum_ever'] = df.groupby(['team'])['goals_against'].apply(lambda x: x.cumsum())
...
ewigetabelle[['rank',
              ...
              'goal_diff_ever',
              'points_cum_ever']].to_excel('ewigetabelle.xls')
