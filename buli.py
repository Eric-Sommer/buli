...
dfhome = dfhome.drop(['hometeam',
                      'awayteam',
                      'homegoals',
                      'awaygoals'], 1)
dfaway = dfaway.drop(['hometeam',
                      'awayteam',
                      'homegoals',
                      'awaygoals'], 1)
dfhome['home'] = 1
...
df = df.sort_values(by=['season', 'spieltag', 'points_cum', 'goal_diff'],
...
df.to_json(path+'buli_final.json')


print("Punktzahl aller Zweitplatzierten am ", spieltag, ".Spieltag")
zweite = df[['season', 'team', 'points_cum']][(df['spieltag'] == spieltag) &
                                              (df['rank'] == 2)]
...
export = df[['season',
             ...
             'end_rank',
             'diff16',
             'diff17']][(df.spieltag == spieltag) &
                        (df.points_cum == scf_points)]
print(export)

with open(os.path.join(path, 'bl_hist_sp' + str(spieltag) +
                            'pt' + str(scf_points) + '.txt'), 'w') as outfile:
...
for i in plotcases.index:
    ranklist.append(df['rank'][(df['season'] == plotcases['season'][i]) &
                               (df['team'] == plotcases['team'][i]) &
...
for i in plotcases.index:
    print(i)
    plt.plot(spieltagplot,
             plotcases['ranklist'][i],
             label=plotcases['team'][i]+' '+str(plotcases['season'][i]))

plt.legend(loc='upper center',
           bbox_to_anchor=(spieltagplot[0], -0.05),
           fancybox=True,
           shadow=True,
           ncol=5)
plt.show()
