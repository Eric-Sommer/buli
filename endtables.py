...
def collect():
    ...
    """
    tables = pd.DataFrame(columns=['liga',
                                   'season',
                                   'rank',
                                   'team'])
    ...
    for liga in [1, 2, 3]:
        if liga == 3:
            ...
            firstyear = 1993
        for s in list(range(firstyear, 2019)):
            ...
            html = open(
                "data/league_{}/raw/kicker_{}_{}.html".format(liga,
                                                              s,
                                                              m), "r", encoding="utf-8"
            ).read()
            ...
            onetable = pd.DataFrame({'liga': [liga] * len(clubs),
                                     'season': [s] * len(clubs),
...
df = df.join(df.groupby('team')['liga'].min(),
             on='team',
             how='left',
             rsuffix='_min')
df = df[df['liga_min'] <= 2]
...
comb = pd.DataFrame(list(
        product(
            df['team'].unique(), df['season'].unique())
        ),
        columns=['team', 'season'])
...
p = figure(x_range=out['team'],
           plot_height=500,
           plot_width=1000,
           title=titlestring)
colorlist = np.select([out["team"] != TEAM_HIGHLIGHT,
                       out['team'] == TEAM_HIGHLIGHT],
                     ["blue", "orange"])
